...
:root {
    ...
    /* **The same pills as this user's neovim statusline.** `lvim-hud`'s
       `chrome/init.lua:98-103` paints the mode as a coloured pill — normal
       green, insert red, visual orange, command purple, replace cyan, terminal
       blue — and bru's bar sits under the same eyes on the same screen, so the
       modes they share are the same colour. Asked for 2026-08-07.

       Passthrough takes the terminal blue because that is what it is: the keys
       stop being the browser's and go straight to what is underneath. The five
       modes neovim has no counterpart for take what is left.

       The text colour is `--pill-fg`, which is whichever of the theme's dark or
       light reads better on that pill — the same rule `pill_on` applies in the
       statusline, so the two never disagree about a colour. */
    --statusbar-normal-fg: var(--pill-fg);
    --statusbar-normal-bg: var(--green);
    --statusbar-insert-fg: var(--pill-fg);
    --statusbar-insert-bg: var(--red);
    --statusbar-caret-fg: var(--pill-fg);
    --statusbar-caret-bg: var(--orange);
    --statusbar-command-fg: var(--pill-fg);
    --statusbar-command-bg: var(--purple);
    --statusbar-passthrough-fg: var(--pill-fg);
    --statusbar-passthrough-bg: var(--blue);
    --statusbar-hint-fg: var(--pill-fg);
    --statusbar-hint-bg: var(--yellow);
    --statusbar-set-mark-fg: var(--pill-fg);
    --statusbar-set-mark-bg: var(--cyan);
    --statusbar-jump-mark-fg: var(--pill-fg);
    --statusbar-jump-mark-bg: var(--cyan);
    --statusbar-record-macro-fg: var(--pill-fg);
    --statusbar-record-macro-bg: var(--magenta);
    --statusbar-run-macro-fg: var(--pill-fg);
    --statusbar-run-macro-bg: var(--teal);
    ...
    --statusbar-caret-selection-fg: var(--pill-fg);
