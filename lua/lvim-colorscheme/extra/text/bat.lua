...
-- The theme uuid is DERIVED from the theme name, not random: a random one
-- changed on every run, so regenerating rewrote all 48 files even when not a
-- single colour had moved. bat only needs the value to be stable and unique
-- per theme, which a hash of the name gives.
local function stable_uuid(name)
    local h1, h2, h3, h4 = 5381, 52711, 1000003, 99991
    for i = 1, #name do
        local b = name:byte(i)
        h1 = (h1 * 33 + b) % 0x100000000
        h2 = (h2 * 31 + b * i) % 0x100000000
        h3 = (h3 * 37 + b * (i + 7)) % 0x100000000
        h4 = (h4 * 41 + b * (i + 13)) % 0x100000000
    end
...
local function create_scope_element(name, scope, settings, indent)
    ...
    -- Sorted: pairs() has no defined order, so the same palette produced a
    -- different file on every run and every regeneration looked like 167
    -- changed lines that were only a reshuffle.
...
function M.generate(colors)
    ...
    if colors._style:match("_light$") then
        background = "light"
    end
    ...
    local theme = {
        ...
        string.format("        <key>uuid</key>\n        <string>%s</string>", stable_uuid(colors._style_name or colors._name or "lvim")),
        "        <key>settings</key>\n        <array>",
    }
